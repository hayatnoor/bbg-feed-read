    ...
    callback(null, _.map(articles,
      function(art) {
        ...
        var obj = {
          ...
          , streamUrl:      child_by_name(art, "enclosure").attributes.url
          , img:       child_by_name(art, "itunes:image").attributes.href
         // , duration:  child_by_name(art, "itunes:duration").children[0]
          , feed:      meta
          };
...
FeedRead.rss = function(xml, source, callback) {
  ...
  parser.onclosetag = function(tagname, current_tag) {
    if (tagname == "item") {
      ...
      meta.description = child_data(current_tag, "description");
  ...
  parser.onend = function() {
    callback(null, _.map(articles,
      function(art) {
          var streamURL = '';
          if (child_by_name(art, "enclosure")) {
            streamURL = child_by_name(art, "enclosure").attributes.url
          }

          var description = '';
          if (!scrub_html(child_data(art, "description")) || scrub_html(child_data(art, "description")).length === 0) {
          ...
          var obj = {
            title:     child_data(art, "title")
            , content:   description // || scrub_html(child_data(art, "content:encoded"))
            , published: child_data(art, "pubDate")
            , author:    child_data(art, "author")
            || child_data(art, "dc:creator")
            , streamURL:       streamURL
            , img:        child_by_name(art, "itunes:image")
        //    , duration:   child_by_name(art, "itunes:duration").children[0]
            , feed:      meta
          };
...
function scrub_html(html) {
  // if there's a closing paragraph tag, grab the first part of the text up to the end of the paragraph tag
  if (html.indexOf('</p>') > -1) {
    html = html.substring(0, html.indexOf('</p>')) + '</p>';
  }
...
function child_by_name(parent, name) {
  if (name.indexOf('image') > 1) {
    ...
    for (var i = 0; i < children.length; i++) {
      if (children[i].name == name) return children[i].attributes.href;
    }
    return null;
  } else {
    var children = parent.children || [];
